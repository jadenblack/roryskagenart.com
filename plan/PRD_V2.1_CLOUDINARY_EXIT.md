# Product Requirements Document (PRD): Cloudinary Exit — Consolidation onto Supabase Storage (v2.1 – v2.4)
...
Exit Cloudinary entirely and consolidate all media serving, storage, and upload onto the Supabase project the application already depends on for its database and authentication. After v2.4 the application has exactly **one external vendor dependency** (Supabase) for data, auth, and media.
...
### 2.2 Storage layout and access model
...
### 2.3 Dual-read cutover (no broken images, ever)
...
2. Miss → existing SVG fallback.

Cloudinary is deleted only after the stage-2.4 gate: **zero registry misses for 7 consecutive days** in production logs.
...
| :--- | :--- | :--- | :--- | :--- |
| 1 | v2.1 | Migration script + bucket + `media_assets` registry populated | — (additive) | none |
| 2 | v2.2 | `assetRegistry.ts` + resolution-chain swap + rendition wiring in UI | nothing yet (dual-read) | v2.1 |
| 3 | v2.3 | Direct browser→Storage uploads + Studio Media Manager view | `/api/cloudinary/upload` proxy | v2.2, admin auth middleware |
| 4 | v2.4 | Cloudinary deletion pass | everything in §2 scope list | v2.3 + 7-day serving gate |

---

## 5. Stage Detail

### v2.1 — Migration Script, Bucket, and Registry Population
...
- `scripts/migrate-cloudinary-to-supabase.ts` (~200 lines, tsx-runnable):
  ...
  2. Download original → `sharp` render the four renditions (§2.1).
  4. Upload to the bucket layout (§2.2).
...
### v2.2 — Registry Swap and Rendition Wiring
...
- Rewire the exact 4 call sites (B4) to consume `assetRegistry` first, then the frozen Cloudinary path, then SVG fallback — per §2.2.
...
- Miss-logging shim (`console.warn('[asset-migration] cloudinary fallback:', slug)`) behind a flag, grep-able for the v2.4 gate.
...
- **Admin auth middleware (v2 audit finding #1) must land before v2.3** — media mutation endpoints must never be anonymous. v2.1 and v2.2 are safe without it (read-only serving + script-time service-role writes).
...
- `src/data/mediaAssetsData.ts` (B8) is deleted in v2.2 when `assetRegistry` replaces it — no migration of its stale content.
- One intentional follow-up to scope in stage v2.2: `tsc --noEmit` flags an existing unused-variable error in `CloudinaryManager.tsx` that predates this PRD; the v2.3 replacement of that view resolves it naturally, so stages v2.1–v2.2 should ship with that known finding unless an earlier cleanup pass prefers to fix it.
...
| Full rollback | Stage 2 is a resolution-order change — reverting one import restores Cloudinary-first behavior; nothing is deleted until v2.4 |

## 8. Out of Scope

- Runtime on-the-fly transforms (rejected deliberately — see §1.2).
- Video/media other than static images (catalog has none).
- Admin authentication (separate remediation PRD, hard dependency only for v2.3).
