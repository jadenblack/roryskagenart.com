...
| [`PROMPT_GIT_REF_PLUMBING_FIX.md`](./PROMPT_GIT_REF_PLUMBING_FIX.md) | 📋 **Proposed — ready to run; run this one FIRST** | Hand-off brief for the recurring **Git ref-plumbing failure**: new branch refs are created then pruned, leaving an unborn `HEAD` (`git rev-parse HEAD` fails while `git log <sha>` and `git push <sha>:refs/heads/x` work) | Written 2026-09-15 after the failure recurred across three consecutive sessions. Sequenced read-only diagnostics, then ranked fixes (Defender exclusion → sync-scope removal → `fsmonitor`/`untrackedCache` off → relocate → `extensions.refStorage reftable` last). ⚠️ **Carries no `V<version>` segment by design** — it is an environment defect that blocks release mechanics, not a release deliverable. ⚠️ **The only prompt here that may modify `.git/`** |
